...
- [ ] Derive 3–7 Key Questions from the model; root each in a root cause, evidence gap, or challenged assumption; for each question record Targets (which sub-problem / root cause / gap it addresses), Unlocks (what becomes possible once answered), Type (Diagnostic / Decision / Risk), and Priority (High / Medium based on impact × uncertainty)
...
- [ ] Read `<session-folder>/session.md` for full context — `## Clarification`, `## Analysis`, the full `## Plan` body, and any sub-bullet notes left under prior ticked `## Implementation` items
...
- [ ] Read `<session-folder>/session.md` end-to-end — `## Clarification` for success criteria (and Reproduction Steps + Root Cause when present), `## Analysis` when present, `## Plan` for the Docs / Source Code / Tests to Change tables, and every ticked item under `## Implementation` for files touched + verification results + any deviation notes
