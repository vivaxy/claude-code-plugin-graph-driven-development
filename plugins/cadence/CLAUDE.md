...
- **Always add new cross-agent behavioral rules to all 4 templates** (`plugins/cadence/templates/*.md`), not to `SKILL.md`, `agents/*.md`, or `CLAUDE.md` — templates are the session-start documents every agent reads as context; the other files serve different purposes
- **Always centralize external references** in `plugins/cadence/README.md` — installation steps, marketplace links, GitHub URLs, version notes, and marketing-style pitches belong only in the README. Files under `plugins/cadence/agents/` and `plugins/cadence/skills/` may freely cross-reference other internal artifacts (other agents, skills, templates, `session.md` sections, `docs/` files, project-level `CLAUDE.md`/`AGENTS.md`/`README.md`) — those internal cross-mentions are required for the agents to function and are not what this rule restricts.
